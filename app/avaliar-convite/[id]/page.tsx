...
// Quase idêntica a app/avaliar/[lead_id]/page.tsx (mesmo desenho visual e
// mesma UX) — separada de propósito porque a fonte de dados é diferente
// (convite direto, sem "lead" nenhum por trás) e a rota de API também
// (/api/review-invites/[id], nunca /api/reviews).
export default function AvaliarConvitePage() {
  const { id } = useParams()
  ...
  useEffect(() => {
    async function load() {
      const res = await fetch(`/api/review-invites/${id}${token ? `?token=${token}` : ''}`)
      if (!res.ok) { setLoading(false); return }
      const json = await res.json()
      setInvite(json.invite)
      setProfessional(json.professional)
      setAlreadyReviewed(json.already_reviewed)
      setName(json.invite?.client_name || '')
      setLoading(false)
    }
    load()
  }, [id, token])
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const res = await fetch(`/api/review-invites/${id}`, {
      ...
      body: JSON.stringify({ rating, comment, client_name: name, token }),
    })
    const json = await res.json()
  ...
  if (!invite || !professional) return (
    <div className="min-h-screen flex items-center justify-center px-6" style={{ background: '#0a0c1a' }}>
      <div className="text-center">
        ...
        <p className="text-gray-500 mt-2">Este convite não existe ou expirou.</p>
      </div>
    </div>
  )
    ...
    <div className="min-h-screen" style={{ background: '#0a0c1a' }}>
      ...
      <div className="max-w-lg mx-auto px-6 py-8">
        ...
        <p className="text-gray-400 text-sm mb-8">
          {professional.name} convidou-te a avaliar um trabalho — a tua avaliação aparece no perfil público dele e ajuda outros clientes.
        </p>
